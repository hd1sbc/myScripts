...
b(){
	# Vars
	local FF="${funcstack[1]}"

	if [ ! -z "${cdBackStack[1]}" ]; then #if back stack isn't empty
		backDir="${cdBackStack[1]}" #dir to move to (1st element in back stack)
		builtin cd "$backDir"
		if [ $? -eq 0 ]; then
		...
		echo "===$F_CDSTACK - $FF===	cd backwards stack is empty; no directory to move to. Returning -1"
		return -1
	fi
}

# This function takes the user forward a dir
f(){
	# Vars
	local FF="${funcstack[1]}"

	if [ ! -z "${cdForwardStack[1]}" ]; then #if forward stack isn't empty
		forwardDir="${cdForwardStack[1]}" #dir to move to (1st element in forward stack)
		...
		echo "===$F_CDSTACK - $FF===	cd forwards stack is empty; no directory to move to. Returning -1"
		return -1
	fi
}
