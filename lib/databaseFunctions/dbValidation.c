...
char* F_DBVAL = (char*) __FILE__;
//-------------------------------------------------------------------------------------------------

// INSTANTIATING GLOBAL VARS-----------------------------------------------------------------------
__attribute__((constructor)) void init_DBVAL() {
...
__attribute__((destructor)) void deinit_DBVAL() {
	free(filesAndFoldersCsvLoc); free(filesAndFoldersCsvBakLoc); free(tableColTypeListLoc);
}
//-------------------------------------------------------------------------------------------------
...
char** getDataTypesArr(char* filePathIn){
	...
	// Data Validation
	if (filePathIn == NULL){ myPerror(F_DBVAL, FF, "Invalid 1st parameter; value is null. Returning null"); return NULL;}
	
	// Getting Values--------------------------------------------------------------------------
	tableColTypeAsStr = convertBasic(tableColTypeListLoc, "string");
	if (tableColTypeAsStr == NULL){ myPerror(F_DBVAL, FF, "convertBasic() function returned a null value. Returning null"); return NULL;}
	...
	if (numRowsInTableColType < 0){
		...
		myPerror(F_DBVAL, FF, "getNumRows() function returned an error val (<0). Returning null");
		return NULL;
	}
	...
	for (int i = 1; i < numRowsInTableColType; i++){
		...
		if (currRow == NULL){
			myPerror(F_DBVAL, FF, "getRow() function returned a null value. Returning null");
			myFree(returnDataTypes); free(tableColTypeAsStr);
			return NULL;
		}
		if (strlen(currRow) == 0){ myPerror(F_DBVAL, FF, "WARNING: getRow() function returned an empty string. Program will continue but errors will likely occur");}

		// Get the file path from that row
		char* filePathCol = getCol(filePathsColTypeListColNum, currRow);
		if (filePathCol == NULL){
			myPerror(F_DBVAL, FF, "getCol() function returned a null value. Returning null");
		...
		if (strcmp(filePathCol, filePathIn) == 0){
			...
			if (returnDataTypes == NULL){ //checking if failure to realloc
				myPerror(F_DBVAL, FF, "myRealloc() function returned a null value. Returning null");
...
int compareDbValues(const char* filePathIn, char* strIn){
	...
	// Data Validation
	if (filePathIn == NULL){ myPerror(F_DBVAL, FF, "Invalid 1st parameter (file path); value is null. Returning -1"); return -1;}
	if (strIn == NULL){ myPerror(F_DBVAL, FF, "Invalid 2nd parameter (new file contents); value is null. Returning -1"); return -1;}
	
	// Getting Values--------------------------------------------------------------------------
	dataTypes = getDataTypesArr((char*) filePathIn);
	if (dataTypes == NULL){
		myPerror(F_DBVAL, FF, "getDataTypesArr() function returned a null value. Returning -1");
		return -1;
	}
	...
	else {fileAsStr = convertBasic(filePathIn, "string");}
	if (fileAsStr == NULL){
		myFree(dataTypes);
		myPerror(F_DBVAL, FF, "convertBasic() function returned a null value. Returning -1");
		return -1;
	}
	...
	if (numRows < 0){
		...
		myPerror(F_DBVAL, FF, "getNumRows() function returned an error val (<0). Returning -1");
		return -1;
	}
	...
	if (numCols < 0){
		free(fileAsStr); myFree(dataTypes);
		myPerror(F_DBVAL, FF, "getNumCols() function returned an error val (<0). Returning -1");
		return -1;
	}
	if (numCols != getArrSize(dataTypes)){
		free(fileAsStr); myFree(dataTypes);
		//keep as returning 0. Not an issue with functions, database isn't valid
		myPerror(F_DBVAL, FF, "getNumCols() returned a different number of columns from what was found in the tableColType csv. Returning 0");
		return 0;
	}
	//-----------------------------------------------------------------------------------------
	...
	for (int i = 1; i < numRows; i++){
		...
		if (currRow == NULL){
			free(fileAsStr); myFree(dataTypes);
			myPerror(F_DBVAL, FF, "getRow() function returned a null value. Returning -1");
			return -1;
		}
		...
		for (int j = 0; j < numCols; j++){
			...
			if (currCol == NULL){
				...
				myPerror(F_DBVAL, FF, "getCol() function returned a null value. Returning 0");
				return 0;
			}
			
			// Checking if int
			if (strcmp(dataTypes[j], "int") == 0 && isInt(currCol) != 1){
				fprintf(stderr, "%s%s%s%s%s", "===", F_DBVAL, " - ", FF, "===	");
...
int isValidDb(char* filePathIn, char* strIn){
	...
	returnVal = compareDbValues(filePathIn, strIn);
	if (returnVal < 0){ myPerror(F_DBVAL, FF, "compareDbValues() returned an error value (<0). Returning -1"); return -1;}
